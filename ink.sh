...
initialize_draft() {
        ...
        if [[ $(git branch | grep 'ink_drafts') == "" ]]; then
...
open_file_name() {
        ...
        then
                ...
                if [ ! -f source/_posts/$FILENAME ]; then #if this IS a new draft
...
load_ink_drafts() {
        ...
        if [ ! -f $FILENAME ]; then # if user entry does not match an existing draft file name.
                echo ''
